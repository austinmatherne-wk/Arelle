...
def unit_denominator(
    ...
    p: OperationDef,
    args: XPathContext.ResultStack,
) -> list[Any] | Sequence[ModelObject | PrototypeObject]:
    if len(args) != 1:
        raise XPathContext.FunctionNumArgs()
    if len(args[0]) != 1:
        raise XPathContext.FunctionArgType(1, "xbrl:unit")
...
def segment(
    ...
    p: OperationDef,
    args: XPathContext.ResultStack,
) -> ModelObject | PrototypeObject | tuple[()]:
...
def infer_precision_decimals(
    ...
    if len(args) != 1:
        raise XPathContext.FunctionNumArgs()
    if len(args[0]) != 1:
        raise XPathContext.FunctionArgType(1, "xbrl:item", args[0])  # type: ignore[arg-type]
...
def checkXffFunctionUse(xc: XPathContext.XPathContext, p: OperationDef, functionName: str) -> None:
    ...
    if xc.progHeader is not None and xc.progHeader.element is not None:
        ...
        if modelResourceElt is None or modelResourceElt.localName not in (
            "formula",
            ...
            raise XPathContext.XPathException(
                ...
                _("Function xff:uncovered-aspect cannot be used on an XPath expression associated with a {0}").format(
                    xc.progHeader.element.localName
                ),
            )
...
def uncovered_non_dimensional_aspects(
    xc: XPathContext.XPathContext,
    p: OperationDef,
    args: XPathContext.ResultStack,
) -> list[Any]:
...
def uncovered_dimensional_aspects(
    xc: XPathContext.XPathContext,
    p: OperationDef,
    args: XPathContext.ResultStack,
) -> list[Any]:
...
def uncovered_aspects(
    xc: XPathContext.XPathContext,
    p: OperationDef,
    args: XPathContext.ResultStack,
    dimensionAspects: bool = False,
) -> list[Any]:
    ...
    return [
        (a if dimensionAspects else aspectToToken.get(a))
        for a in uncoveredAspects
        if a != Aspect.DIMENSIONS and isinstance(a, QName) == dimensionAspects
    ]
...
def nodesEqual(
    ...
    mustBeItems: bool = False,
    nonItemErrCode: str | None = None,
) -> bool:
    if len(args) != 2:
        raise XPathContext.FunctionNumArgs()
...
def s_equal_test(node1: ModelObject | ModelAttribute, node2: ModelObject | ModelAttribute) -> bool:
    if (
        ...
        and node1.isItem
        and isinstance(node2, (ModelFact, ModelInlineFact))
        and node2.isItem
    ):
        return (
            c_equal_test(node1, node2)
            and u_equal_test(node1, node2)
            and XbrlUtil.xEqual(node1, node2)
            # must be validated (by xEqual) before precision tests to assure xAttributes is set
            and node1.xAttributes.get("precision") == node2.xAttributes.get("precision")
            and node1.xAttributes.get("decimals") == node2.xAttributes.get("decimals")
        )
    elif isinstance(node1, ModelObject):
        if isinstance(node2, ModelObject):
            return XbrlUtil.sEqual(
                cast(ModelXbrl, node1.modelXbrl),
                node1,
                node2,
                excludeIDs=XbrlUtil.TOP_IDs_EXCLUDED,
                dts2=node2.modelXbrl,
            )
        else:
            return False
...
def duplicate_tuple_test(
    node1: ModelFact | ModelInlineFact,
    node2: ModelFact | ModelInlineFact,
    topLevel: bool = True,
) -> bool:
...
def pcu_equal_test(modelItem1: Any, modelItem2: Any) -> bool:
    return (
        p_equal_test(modelItem1, modelItem2)
        and c_equal_test(modelItem1, modelItem2)
        and u_equal_test(modelItem1, modelItem2)
    )
...
def concept(xc: XPathContext.XPathContext, p: OperationDef, args: XPathContext.ResultStack) -> ModelConcept:
    ...
    if (
        srcConcept is None
        or not (srcConcept.isItem or srcConcept.isTuple)
        or srcConcept.qname is None
        or srcConcept.qname.namespaceURI == XbrlConst.xbrli
    ):
        raise XPathContext.XPathException(
            p,
            "xfie:invalidConceptQName",
...
def filter_member_network_selection(
    xc: XPathContext.XPathContext,
    p: OperationDef,
    args: XPathContext.ResultStack,
) -> set[Any] | tuple[()]:
...
def filter_member_network_members(
    relationshipSet: Any,
    fromRels: Any,
    recurse: bool,
    members: set[Any] | None = None,
    relationships: set[Any] | None = None,
    linkQnames: set[Any] | None = None,
    arcQnames: set[Any] | None = None,
) -> None:
    if members is None:
        members = set()
        ...
        if toConceptQname not in members:
            ...
            if recurse:
                filter_member_network_members(
                    relationshipSet,
                    relationshipSet.fromModelObject(toConcept),
                    recurse,
                    members,
                    relationships,
                    linkQnames,
...
def filter_member_DRS_selection(
    xc: XPathContext.XPathContext,
    p: OperationDef,
    args: XPathContext.ResultStack,
) -> set[Any] | tuple[()]:
    ...
    if dimConcept is None or dimConcept.qname is None or dimConcept.qname.namespaceURI == XbrlConst.xbrli:
        raise XPathContext.XPathException(
            p,
            "xfie:invalidDimensionQName",
            _("Argument 1 {0} is not in the DTS.").format(qnDim),
        )
    ...
    if priItemConcept is None or priItemConcept.qname is None or priItemConcept.qname.namespaceURI == XbrlConst.xbrli:
        raise XPathContext.XPathException(
            p,
            "xfie:invalidPrimaryItemConceptQName",
            _("Argument 2 {0} is not in the DTS.").format(qnPriItem),
        )
    elif not priItemConcept.isPrimaryItem:
        raise XPathContext.XPathException(
            p,
            "xfie:invalidPrimaryItemConceptQName",
            _("Argument 2 {0} is not a primary item.").format(qnPriItem),
        )
    memConcept = xc.modelXbrl.qnameConcepts.get(qnMem)
...
def filter_member_DRS_members(
    xc: XPathContext.XPathContext,
    fromRels: Any,
    axis: str,
    memConcept: ModelConcept,
    inSelection: bool,
    visited: set[Any],
    memSelectionQnames: set[Any],
) -> None:
    for rel in fromRels:
...
def fact_has_explicit_dimension_value(
    xc: XPathContext.XPathContext,
    p: OperationDef,
    args: XPathContext.ResultStack,
) -> bool:
    if len(args) != 3:
        raise XPathContext.FunctionNumArgs()
    return cast(QName, qnameArg(xc, p, args, 2, "QName", emptyFallback=())) == fact_explicit_dimension_value_value(
        xc, p, args
    )


def fact_explicit_dimension_value(
    xc: XPathContext.XPathContext,
    p: OperationDef,
    args: XPathContext.ResultStack,
) -> Any:
...
def fact_typed_dimension_simple_value(
    xc: XPathContext.XPathContext,
    p: OperationDef,
    args: XPathContext.ResultStack,
) -> Any:
...
def fact_explicit_dimensions(
    ...
    if context is not None:
        return (
            set(qn for qn, dim in context.qnameDims.items() if dim.isExplicit)
...
def concept_relationships(
    ...
    p: OperationDef,
    args: XPathContext.ResultStack,
    nestResults: bool = False,
    targetRole: bool = False,
) -> Any:  # nestResults if for table linkbase relationships
    lenArgs = len(args)
    if not 4 <= lenArgs <= 8:
        raise XPathContext.FunctionNumArgs()
    ...
    if lenArgs > 4:
        ...
        if axis in ("child", "parent", "sibling", "sibling-or-self") and generations != 1:
            raise XPathContext.FunctionArgType(
                4,
                "generations must be 1 for 'child', 'parent', 'sibling' or 'sibling-or-self' axis",
                errCode="xfie:InvalidConceptRelationParameters",
                value=generations,
            )
                ...
                if qnSource in rootQNs:
                    ...
                    if indexOfSelfInRoots >= 0:
                        ...
                        if result:
                            ...
                            if axis == "sibling-or-descendant-or-self":
                                return (
                                    rootQNs[:indexOfSelfInRoots]
                                    + [qnSource]
                                    + result
                                    + rootQNs[indexOfSelfInRoots + 1 :]
                                )
                return []
...
def concept_relationships_step(
    xc: XPathContext.XPathContext,
    inst: ModelXbrl,
    relationshipSet: Any,
    rels: Any,
    ...
    if rels:
        for modelRel in rels:
            ...
            if generations > 1 or (generations == 0 and conceptQname not in visited):
                ...
                if axis == "descendant":
                    if targetRole:
                        stepRelationshipSet = inst.relationshipSet(
                            relationshipSet.arcrole,
                            modelRel.consecutiveLinkrole,
                        )
                    else:
...
def distinct_nonAbstract_parent_concepts(
    xc: XPathContext.XPathContext,
    p: OperationDef,
    args: XPathContext.ResultStack,
) -> set[QName]:
    ...
    if relationshipSet:
        for rel in relationshipSet.modelRelationships:
            ...
            if (
                isinstance(fromModelObject, ModelConcept)
                and isinstance(toModelObject, ModelConcept)
                and not fromModelObject.isAbstract
...
def create_element(xc: XPathContext.XPathContext, p: OperationDef, args: XPathContext.ResultStack) -> ModelObject:
    ...
    if attrArg:
        if len(attrArg) & 1 or any(
            (not isinstance(arg, (QName, str))) or (isinstance(arg, str) and NCNamePattern.match(arg) is None)
            ...
            raise XPathContext.FunctionArgType(
                ...
                errCode="xfie:AttributesNotNameValuePairs",
            )
        else:
            attrParam = [
                (attrArg[i], attrArg[i + 1])  # need name-value pairs for XmlUtil function
                for i in range(0, len(attrArg), 2)
            ]
    else:
        attrParam = None
...
def filingIndicatorValues(inst: ModelXbrl, filedValue: str) -> set[str]:
    ...
    for fact in inst.factsByQname[XbrlConst.qnFiFact]:
        if (
            ...
            and XbrlConst.qnFiDim in fact.context.qnameDims
            and fact.value.strip() == filedValue
        ):
...
def positive_filing_indicators(
    xc: XPathContext.XPathContext,
    p: OperationDef,
    args: XPathContext.ResultStack,
) -> list[str]:
...
def negative_filing_indicators(
    xc: XPathContext.XPathContext,
    p: OperationDef,
    args: XPathContext.ResultStack,
) -> list[str]:
