            ...
            if importToDTS:
                ...
                if reportPackage := filesource.reportPackage:
                    for report in reportPackage.reports:
                        reportEntries = report.fullPathFiles
                        if report.isInline and len(report.files) > 1:
                            ...
                            for pluginXbrlMethod in pluginClassMethods("InlineDocumentSet.Discovery"):
                                ...
                                ixdsDiscovered = True
                            if not ixdsDiscovered:
                                self.addToLog(_("Inline document set encountered. Enable 'Inline XBRL Document Set' plug-in to load this filing: {0}").format(filesource.url))
                                continue
                        filesource.select(reportEntries)
