                ...
                if report.isInline:
                    ...
                    if not ixdsDiscovered:
                        if len(reportEntries) > 1:
                            raise RuntimeError(_("Inline document set encountered. Enable 'InlineDocumentSet' plug-in to load this filing: {0}").format(filesource.url))
                        reportEntries = [{"file": report.fullPathPrimary}]
