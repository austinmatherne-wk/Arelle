...
verbatim, against the plugin the website displays, so the published rule, the
published command and the published output cannot drift apart.
"""
...
args = parse_args(
    this_file.stem,
    "Confirm the arelle.org plugin example still produces its published output.",
)
