...
1. Install a pre-built application. ([See Quick Start](/arelle/pub/edgar-renderer-installation/))
2. Install from sources on GitHub.

Once installed there are three strategies for using the EDGAR renderer:

1. Interactively from its GUI. ([See Quick Start](/arelle/pub/edgar-renderer-installation/))
...
- Python libraries for: lxml, numpy, isodate, regex, openpyxl, pyparsing, Pillow, pywin32 (Windows only), dateutil, holidays, matplotlib. (You may use pip, easy_install, or Windows binaries on <http://www.lfd.uci.edu/~gohlke/pythonlibs/>.) Plugins may also require graphviz, pg8000, pycountry and others (per authors of plugins).
- Arelle® [source code on GitHub (master branch suggested)](https://github.com/Arelle/Arelle)
- EDGAR Renderer [source code on GitHub (master branch suggested)](https://github.com/Arelle/EDGAR). Suggested installation location is under Arelle's plugin directory — `{installed Arelle location}/arelle/plugin/EdgarRenderer`
...
The `edgar_renderer_3_3_0_814.zip` distribution described below is no longer published. Use the current EDGAR Renderer from the [Arelle/EDGAR repository](https://github.com/Arelle/EDGAR) instead. The remainder of this section is retained as a record of how that distribution was laid out.
...
Developer issue management is by the Jira EDGAR Renderer project: [https://arelle.atlassian.net/projects/ER](https://arelle.atlassian.net/projects/ER)
